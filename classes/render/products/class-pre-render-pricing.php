...
class Pre_Render_Pricing
{
	...
	*/
    public function wps_products_price($user_params)
    {
        return $this->get_pricing_template(
            $this->gather_products_pricing_params($user_params)
        );
    }
	...
	*/
    public function get_multi_price_markup($first_price, $last_price)
    {
        return apply_filters(
            ...
            '<small class="wps-product-from-price">' .
                __('From: ', 'wpshopify') .
                '</small>'
        ) .
    ...
    {
        return $show_price_range &&
            $this->Money->has_more_than_one_price($variants_amount);
    }
	...
	*/
    public function build_pricing_params($params)
    {
        ...
        $variants_in_stock = $this->DB_Variants->get_in_stock_variants_from_product(
            $product
        );
        ...
        if (!empty($params['data']['show_compare_at'])) {
            if (!empty($params['data']['show_price_range'])) {
                ...
                $variants_sorted = $this->DB_Variants->sort_variants_by_position(
                    $variants_in_stock
                );
            }
        ...
        $asdfisjd = !empty($params['data']['show_price_range'])
            ? $params['data']['show_price_range']
            : false;

        if ($this->is_real_price_range($asdfisjd, $variants_amount)) {
            $price_markup = $this->get_multi_price_markup(
        ...
        return $this->Render_Defaults->product_pricing([
            'price' => $price_markup,
            'show_compare_at' => isset($params['data']['show_compare_at'])
                ? (int) $params['data']['show_compare_at']
                : false,
            'show_local' => isset($params['data']['show_local'])
                ? (int) $params['data']['show_local']
                : false,
            'show_price_range' => isset($params['data']['show_price_range'])
                ? (int) $params['data']['show_price_range']
                : false,
            ...
            'product' => $product,
            'custom' => isset($params['data']['custom'])
                ? $params['data']['custom']
                : false,
            'render_from_server' => isset($params['data']['render_from_server'])
                ? $params['data']['render_from_server']
                : false,
        ]);
    }
    ...
    {
        return $this->DB_Variants->check_if_all_variant_prices_match(
            $last_price,
            $first_price
        );
    }
    ...
    public function pre_render_product_pricing($params)
    {
		...
		*/
        if ($this->params_show_compare_at($params)) {
            $final_params_2 = $this->wps_products_price(
                $this->toggle_show_compare_at($params)
            );

            $returning_params['data'] = [
                $final_params_1['data'],
                $final_params_2['data'],
            ];
        } else {
