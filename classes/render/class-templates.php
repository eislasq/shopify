...
class Templates
{
    ...
    */
    ...
    {
        if (
            !$this->has_pre_render_method($params) ||
            !$this->has_params_data($params)
        ) {
            return $this->set_and_get_template(
                $this->params_no_results($params)
        ...
        if (is_wp_error($normalized_params)) {
            return $this->set_and_get_template(
                $this->params_wp_error($params, $normalized_params)
            );
        }

        $all_params = $this->get_params_for_load($normalized_params, $params);

        return $this->load_many_wrapper(
            $all_params['params'],
   ...
   [price] => '<span></span>',
    ...
    public function load_no_pre_render($params)
    {
        ...
        if (is_array($user_params['post_id'])) {
    ...
    public function load($params)
    {
        ...
        if (
            !isset($params['data']['render_from_server']) ||
        ...
        $ids = $this->Render_Data->get_ids_from_shortcode(
            $params['data'],
            $params['type']
        );
