...
class Data
{
	...
	*/
    public function get_products_data_from_products_id($product_id = false)
    {
        ...
        $product->images = $this->DB_Images->get_images_from_product_id(
            $product_id
        );
        $product->tags = $this->DB_Tags->construct_only_tag_names(
            $this->DB_Tags->get_tags_from_product_id($product_id)
        );
        $product->variants = $this->DB_Variants->get_in_stock_variants_from_product_id(
            $product_id
        );
        $product->options = $this->DB_Options->get_options_from_product_id(
            $product_id
        );
        $product->collections = $this->DB_Collections->get_collections_by_product_id(
            $product->product_id
        );
    ...
    public function get_defaults($params, $required_data)
    {
        ...
        return $this->Render_Products_Defaults->{$method_name}(
            $required_data[$params['data_type']]
        );
    }
	...
	*/
    public function normalize($params)
    {
        ...
        $required_data = $this->get_required_data(
            $params['data'],
            $params['data_type']
        );
        ...
        if (!$required_data || !$params['data']) {
            return new \WP_Error(
                'error',
                __(
                    "Missing data. You probably have Lite sync turned on or need to resync your store",
                    'wpshopify'
                )
            );
        }
    ...
    public function get_data_from_type_id($data, $type)
    {
        ...
        return $this->{'get_' . $type . '_data_from_' . $type . '_id'}(
            $this->get_data_type_id($data, $type)
        );
    }
	...
	*/
    ...
    {
        return $this->has_filter_params(
            $shortcode_attrs,
            $this->product_filter_params()
        );
    }
    ...
    public function has_accumulating_collection_attrs($shortcode_attrs)
    {
        return $this->has_filter_params(
            $shortcode_attrs,
            $this->accumulating_collection_attrs()
        );
    }
    ...
    public function get_ids_from_product_shortcode($atts)
    {
        ...
        if (!$this->has_filter_params_product_attrs($atts)) {
            ...
            $ids['products_id'] = $this->normalize_ids(
                !empty($atts['products_id']) ? $atts['products_id'] : []
            );
        }

        $ids['title'] = !empty($atts['title'])
            ? $this->DB_Products->get_product_ids_from_titles($atts['title'])
            : [];
        $ids['vendor'] = !empty($atts['vendor'])
            ? $this->DB_Products->get_product_ids_from_vendors($atts['vendor'])
            : [];
        $ids['description'] = !empty($atts['description'])
            ? $this->DB_Products->get_product_ids_from_description(
                $atts['description']
            )
            : [];
        $ids['product_type'] = !empty($atts['product_type'])
            ? $this->DB_Products->get_product_ids_from_types(
                $atts['product_type']
            )
            : [];
        $ids['slug'] = !empty($atts['slug'])
            ? $this->DB_Products->get_product_ids_from_handles($atts['slug'])
            : [];
        $ids['tag'] = !empty($atts['tag'])
            ? $this->DB_Tags->get_product_ids_from_tag($atts['tag'])
            : [];
        $ids['post_id'] = !empty($atts['post_id'])
            ? $this->DB_Products->get_product_ids_from_post_ids(
                $atts['post_id']
            )
            : [];
    ...
    public function get_ids_from_collection_shortcode($atts)
    {
        ...
        $ids['title'] = isset($atts['title'])
            ? $this->DB_Collections->get_collection_ids_from_titles(
                $atts['title']
            )
            : false;

        $ids['description'] = isset($atts['description'])
            ? $this->DB_Collections->get_collection_ids_from_description(
                $atts['description']
            )
            : false;

        $ids['post_id'] = isset($atts['post_id'])
            ? $this->DB_Collections->get_collection_ids_from_post_ids(
                $atts['post_id']
            )
            : false;

        $ids['collection_id'] = isset($atts['collection_id'])
            ? $this->normalize_ids($atts['collection_id'])
            : false;
