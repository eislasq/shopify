   ...
   */
    ...
    public function get_column_defaults()
    {
        return [
            ...
            'finished_product_posts_relationships' =>
                $this->default_finished_product_posts_relationships,
            'finished_media' => $this->default_finished_media,
            'finished_collection_posts_relationships' =>
                $this->default_finished_collection_posts_relationships,
    ...
    public function reset_syncing_posts_relationships()
    {
        return [
            'syncing_start_time' => $this->update_column_single(
                ['finished_product_posts_relationships' => 0],
                ['id' => 1]
            ),
            'syncing_end_time' => $this->update_column_single(
                ['finished_collection_posts_relationships' => 0],
                ['id' => 1]
            ),
        ];
    }
    ...
    {
        $finished_product_posts_relationships = $this->get_column_single(
            'finished_product_posts_relationships'
        );

        if (
            Utils::array_not_empty($finished_product_posts_relationships) &&
            isset(
                $finished_product_posts_relationships[0]
                    ->finished_product_posts_relationships
            )
        ) {
            return (bool) $finished_product_posts_relationships[0]
                ->finished_product_posts_relationships;
        } else {
            return false;
        }
    ...
    {
        $finished_collection_posts_relationships = $this->get_column_single(
            'finished_collection_posts_relationships'
        );

        if (
            Utils::array_not_empty($finished_collection_posts_relationships) &&
            isset(
                $finished_collection_posts_relationships[0]
                    ->finished_collection_posts_relationships
            )
        ) {
            return (bool) $finished_collection_posts_relationships[0]
                ->finished_collection_posts_relationships;
        } else {
            return false;
        }
     ...
     */
    public function increment_current_deletion_amount(
        $key,
        $amount_to_increment = false
    ) {
        global $wpdb;
        ...
        $query =
            'UPDATE ' .
            $this->table_name .
            ' SET syncing_current_deletion_amounts_' .
            $key .
            ' = syncing_current_deletion_amounts_' .
            $key .
            ' + ' .
            $amount_to_increment;
    ...
    {
        return $this->update_column_single(
            ['finished_product_posts_relationships' => $status],
            ['id' => 1]
        );
    }
    ...
    {
        return $this->update_column_single(
            ['finished_collection_posts_relationships' => $status],
            ['id' => 1]
        );
    }
    ...
    {
        return $this->update_column_single(
            ['syncing_totals_webhooks' => WP_SHOPIFY_TOTAL_WEBHOOKS_COUNT],
            ['id' => 1]
        );
    }
    ...
    {
        $finished_webhooks_deletions = $this->get_column_single(
            'finished_webhooks_deletions'
        );

        if (
            Utils::array_not_empty($finished_webhooks_deletions) &&
            isset($finished_webhooks_deletions[0]->finished_webhooks_deletions)
        ) {
            return (bool) $finished_webhooks_deletions[0]
                ->finished_webhooks_deletions;
        } else {
            return false;
        }
    ...
    {
        $finished_data_deletions = $this->get_column_single(
            'finished_data_deletions'
        );

        if (
            Utils::array_not_empty($finished_data_deletions) &&
            isset($finished_data_deletions[0]->finished_data_deletions)
        ) {
