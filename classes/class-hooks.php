...
class Hooks
{
    ...
    public function wps_products_custom_args($args)
    {
        return array(
            'items_per_row' => apply_filters(
                'wps_products_custom_args_items_per_row',
                3
            )
        );
    }
    ...
    public function wps_collections_custom_args()
    {
        return array(
            'items_per_row' => apply_filters(
                'wps_collections_custom_args_items_per_row',
                3
            )
        );
    }
    ...
    public function wps_product_single_price_multi(
        $defaultPrice,
        $priceFirst,
        $priceLast,
        $product
    ...
    public function wps_product_single_price_one(
        $defaultPrice,
        $finalPrice,
        $product
    ) {
    ...
    {
        if ($this->DB_Settings_General->products_link_to() === 'wordpress') {
            return 'https://' .
                $this->DB_Shop->domain() .
                '/products/' .
                $product->handle;
     ...
     */
    public function wps_products_show_sidebar()
    {
        return false;
    }
        ...
        if (empty($shortcodeData->shortcodeArgs)) {
            return [
                ...
                'posts_per_page' => apply_filters(
                    'wps_collections_args_posts_per_page',
                    $settingsNumPosts
                ),
                'orderby' => apply_filters(
                    'wps_collections_args_orderby',
                    'desc'
                ),
    ...
    public function wps_products_args($shortcodeData)
    {
        ...
        if (empty($shortcodeData->shortcodeArgs)) {
            return [
                ...
                'post_status' => 'publish',
                'posts_per_page' => apply_filters(
                    'wps_products_args_posts_per_page',
                    $settingsNumPosts
                ),
     ...
     */
    public function wps_product_single_price(
        $default,
        $priceFirst,
        $priceLast,
        $product
        ...
        if ($priceFirst !== $priceLast) {
            $defaultPrice =
                apply_filters(
                    'wps_product_single_price_multi_from',
                    '<small class="wps-product-from-price">From: </small>'
                ) .
                apply_filters(
                    'wps_product_single_price_multi_first',
                    $priceFirst
                ) .
                apply_filters(
                    'wps_product_single_price_multi_separator',
                    ' <span class="wps-product-from-price-separator">-</span> '
                ) .
                apply_filters(
                    'wps_product_single_price_multi_last',
                    $priceLast
                );

            $finalPrice = apply_filters(
                'wps_product_single_price_multi',
                $defaultPrice,
                $priceFirst,
                $priceLast,
                $product
...
<?php
   ...
   */
    ...
    public function wps_products_all_args()
    {
        $args = [];
    ...
    public function wps_products_single_args()
    {
        global $post;

        return [
            'product_id' => $this->DB_Products->get_product_ids_from_post_ids(
                $post->ID
            ),
    ...
    public function wps_products_all_title()
    {
        global $post;
    ...
    public function hooks()
    {
        add_filter(
            'login_redirect',
            [$this, 'wps_customer_login_redirect'],
            10,
            3
        );
        add_action(
        ...
        add_filter('wps_products_show_sidebar', [
            $this,
            'wps_products_show_sidebar'
        ]);

        // add_filter('wps_products_show_sidebar', false);

        add_filter('wps_products_all_args', [$this, 'wps_products_all_args']);
        add_filter('wps_products_single_args', [
            $this,
            'wps_products_single_args'
        ]);

        add_action('wps_product_single_sidebar', [
            $this,
            'wps_product_single_sidebar'
        ]);
        add_action('wps_collections_sidebar', [
            $this,
            'wps_collections_sidebar'
        ]);
        add_action('wps_collection_single_sidebar', [
            $this,
            'wps_collection_single_sidebar'
        ]);

        add_filter('wps_collections_args', [$this, 'wps_collections_args']);
        add_filter('wps_collections_custom_args', [
            $this,
            'wps_collections_custom_args'
        ]);
        add_filter('wps_collections_custom_args_items_per_row', [
            $this,
            'wps_collections_custom_args_items_per_row'
        ]);
        add_filter('wps_collection_single_products_heading_class', [
            $this,
            'wps_collection_single_products_heading_class'
        ]);

        add_filter('wps_products_args', [$this, 'wps_products_args']);
        add_filter('wps_products_args_posts_per_page', [
            $this,
            'wps_products_args_posts_per_page'
        ]);
        add_filter('wps_products_args_orderby', [
            $this,
            'wps_products_args_orderby'
        ]);
        add_filter('wps_products_args_paged', [
            $this,
            'wps_products_args_paged'
        ]);
        add_filter('wps_products_custom_args', [
            $this,
            'wps_products_custom_args'
        ]);
        add_filter('wps_products_custom_args_items_per_row', [
            $this,
            'wps_products_custom_args_items_per_row'
        ]);
        add_filter('wps_products_price', [$this, 'wps_products_price'], 10, 2);

        add_filter('wps_products_heading_show', [
            $this,
            'wps_products_heading_show'
        ]);
        add_filter('wps_collections_heading_show', [
            $this,
            'wps_collections_heading_show'
        ]);
        add_filter('wps_related_products_heading_show', [
            $this,
            'wps_related_products_heading_show'
        ]);

        add_filter(
            'wps_product_single_thumbs_class',
            [$this, 'wps_product_single_thumbs_class'],
            10,
            2
        );
        add_filter(
            'wps_product_single_price',
            [$this, 'wps_product_single_price'],
            10,
            4
        );
        add_filter(
            'wps_product_single_price_multi',
            [$this, 'wps_product_single_price_multi'],
            10,
            4
        );
        add_filter(
            'wps_product_single_price_one',
            [$this, 'wps_product_single_price_one'],
            10,
            3
        );
