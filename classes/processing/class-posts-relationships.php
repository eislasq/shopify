...
class Posts_Relationships extends
    \WP_Shopify\Processing\Vendor_Background_Process
{
	...
	*/
    public function maybe_save_post_warnings($post_results)
    {
        ...
        return array_map(
            [$this, 'save_post_warning'],
            Utils_Data::return_only_wp_errors($post_results)
        );
    }
	...
	*/
    protected function task($post)
    {
        ...
        if ($this->post_type_switched($post)) {
            $this->DB_Settings_Syncing->set_finished_relationship(
                $this->meta['post_type']
            );
	...
	*/
    ...
    {
        if ($this->meta['post_type'] === WP_SHOPIFY_PRODUCTS_POST_TYPE_SLUG) {
            do_action('wps_finished_syncing_products');
        }

        if (
            $this->meta['post_type'] === WP_SHOPIFY_COLLECTIONS_POST_TYPE_SLUG
        ) {
            do_action('wps_finished_syncing_collections');
        }
