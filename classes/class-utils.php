...
class Utils
{
	...
	*/
    public static function emptyConnection($connection)
    {
        if (!is_object($connection)) {
            ...
            if (
                property_exists($connection, 'api_key') &&
                $connection->api_key
            ) {
                return false;
            } else {
	...
	*/
    public static function manually_sort_posts_by_title(
        $sortedArray,
        $unsortedArray
    ) {
        $finalArray = [];
	...
	*/
    ...
    {
        if (
            self::str_contains($parts[0], 'http') ||
            self::str_contains($parts[0], 'https')
        ) {
            if (count($parts) > 1) {
                return $parts[1];
            }
    ...
    public static function check_for_duplicate_folders($wp_path, $abs_wp_path)
    {
        ...
        if (
            $wp_path_exploded[1] === $abs_wp_path_exploded[4] &&
            $wp_path_exploded[2] === end($abs_wp_path_exploded)
        ) {
            // array_pop($abs_wp_path_exploded);
            // $trimmed_array = array_filter($abs_wp_path_exploded);
            // $trimmed_array_imploded = implode("/",$trimmed_array);
	...
	*/
    public static function wps_delete_product_data(
        $postID,
        $type,
        $dataToDelete
    ) {
	...
	*/
    ...
    {
        if (
            property_exists($item, 'published_at') &&
	...
	*/
    ...
    public static function convert_underscore_to_camel_string(
        $string,
    ...
    public static function convert_underscore_to_camel_array(&$array)
    {
        foreach (array_keys($array) as $key):
            ...
            $transformedKey = self::convert_underscore_to_camel_string($key);

            if (is_array($value)) {
                self::convert_underscore_to_camel_array($value);
            }
