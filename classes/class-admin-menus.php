...
class Admin_Menus
{
	...
	*/
    public function products_meta_box_collections($post)
    {
        ...
        if (Utils::has($product, 'product_id')) {
            $collects = $this->DB_Collects->get_collects_from_product_id(
                $product->product_id
            ); ?>
       ...
       $selectedCollection = in_array(
           $collection->collection_id,
           array_column($collects, 'collection_id')
       ); ?>
          ...
          ? 'checked="checked"'
          : '' ?> value="16" type="checkbox" name="collections[<?= $collection->collection_id ?>]" id="collection-<?= $collection->collection_id ?>"> <?php esc_html_e(
    $collection->title
); ?>
					</label>
				</li>
	...
	*/
    public function wps_products_meta_box_tags($post)
    {
      ...
      $selected_tag = in_array(
          $tag->tag,
          array_column($current_tags, 'tag')
      ); ?>
			...
			<li id="tag-wrapper-<?= $tag->tag ?>" class="popular-tag">
				<label class="selectit" for="tag-<?= $tag->tag ?>">
					<input <?= $selected_tag
         ? 'checked="checked"'
         : '' ?> value="<?= $tag->tag_id ?>" type="checkbox" name="tags[<?= $tag->tag ?>]" id="tag-<?= $tag->tag ?>"> <?php esc_html_e(
    $tag->tag
); ?>
				</label>
			</li>
