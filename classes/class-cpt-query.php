...
class CPT_Query
{
	...
	*/
    ...
    public function build_query_from_values($carry, $post, $post_type)
    {
        ...
        $carry .=
            ...
            esc_sql(
                $this->return_guid(
                    $this->construct_post_guid($post, $post_type)
                )
            ) .
            "', " .
            "'" .
	...
	*/
    public function construct_post_case_columns_query($post_values_to_update)
    {
        ...
        foreach ($post_values_to_update as $id => $values) {
            $post_date = esc_sql(
                sanitize_text_field(
                    date('Y-m-d H:i:s', strtotime($values['post_date']))
                )
            );
            $post_date_gmt = esc_sql(
                sanitize_text_field(
                    date('Y-m-d H:i:s', strtotime($values['post_date_gmt']))
                )
            );
            ...
            $post_modified = esc_sql(
                sanitize_text_field(
                    date('Y-m-d H:i:s', strtotime($values['post_modified']))
                )
            );
            $post_modified_gmt = esc_sql(
                sanitize_text_field(
                    date('Y-m-d H:i:s', strtotime($values['post_modified_gmt']))
                )
            );
            ...
            $columns['post_name'] .=
                ...
                $post_name .
                "' ";
            $columns['post_modified'] .=
                "WHEN `id`='" .
                esc_sql(intval($id)) .
                "' THEN '" .
                $post_modified .
                "' ";
            $columns['post_modified_gmt'] .=
                "WHEN `id`='" .
                esc_sql(intval($id)) .
                "' THEN '" .
                $post_modified_gmt .
	...
	*/
    ...
    {
        return " WHERE `id`='" .
            implode("' OR `id`='", array_keys($post_values_to_update)) .
            "'";
    }
	...
	*/
    public function join_query_with_commas(
        $update_query,
        $columns,
        $where_clause
    ) {
	...
	*/
    public function only_posts_with_matching_handle($new_items, $existing_posts)
    {
        return array_filter(
            $new_items,
            function ($item, $index) use ($existing_posts) {
                if (
                    CPT::post_exists_by_handle($existing_posts, $item->handle)
                ) {
                    return true;
                }
            },
	...
	*/
    public function find_posts_to_update($new_items, $post_type)
    {
        ...
        $matching_items = $this->only_posts_with_matching_handle(
            $new_items,
            $existing_posts
        );

        return $this->add_post_id_before_update(
            $matching_items,
            $existing_posts
        );
    }
	...
	*/
    public function format_posts_for_update(
        $shopify_posts_to_update,
        $post_type
    ) {
        $results = [];
	...
	*/
    public function find_posts_to_insert($items_from_shopify, $existing_posts)
    {
        ...
        return array_filter(
            $items_from_shopify,
            function ($item, $index) use ($existing_posts) {
                return !CPT::post_exists_by_handle(
                    $existing_posts,
                    $item->handle
                );
            },
	...
	*/
    public function posts_insert_query(
        $posts_column_names,
        $posts_column_values_insert
    ) {
        global $wpdb;
	...
	*/
    public function construct_posts_insert_query($items, $post_type)
    {
        ...
        $items_to_insert = $this->find_posts_to_insert(
            $items,
            CPT::get_all_posts_compressed($post_type)
        );
        ...
        $posts_column_values_insert = $this->construct_posts_col_values(
            $items_to_insert,
            $post_type
        );

        return $this->posts_insert_query(
            $this->posts_col_names(),
            $posts_column_values_insert
        );
    }
	...
	*/
    public function query($query, $type)
    {
        if ($query) {
            ...
            return $this->DB_Settings_General->sanitize_db_response(
                $insertion_result,
                'Failed to execute query for ' . $type,
                'query'
            );
        }
    }
	...
	*/
    ...
    {
        $query = $this->construct_posts_update_query(
            $this->format_posts_for_update($posts_to_update, $post_type)
        );
	...
	*/
    ...
    {
        $result = $this->CPT_Meta->update_posts_relationship(
            $post,
            $db_class,
            $params
        );
	...
	*/
    public function modify_posts_relationship($post, $params)
    {
        return array_map(function ($relationship) use ($params, $post) {
            return $this->process_post_relationship(
                $relationship,
                $post,
                $params
            );
    ...
    public function modify_posts_relationships($params)
    {
        $results = array_map(function ($post) use ($params) {
            return [
                ...
                'post_meta' => $this->CPT_Meta->update_posts_meta_relationship(
                    $post,
                    $params
                ),
            ];
	...
	*/
    public function modify_posts($params)
    {
        ...
        $result_insert = $this->insert_posts(
            $params['items'],
            $params['post_type']
        );
