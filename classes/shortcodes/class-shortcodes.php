...
class Shortcodes
{
    ...
    public function gather_shortcode_atts($atts, $defaults_method)
    {
        $combined = $this->combine_user_and_default_atts(
            $atts,
            function () use ($atts, $defaults_method) {
    ...
    {
        $ids = [];
        $ids['title'] = $this->DB_Collections->get_collection_ids_from_titles(
            $atts['title']
        );
        $ids[
            'description'
    ...
    public function get_ids_from_product_shortcode($atts)
    {
        ...
        if (!$this->has_filter_params_product_attrs($atts)) {
            ...
            $ids['product_id'] = $this->normalize_ids(
                !empty($atts['product_id']) ? $atts['product_id'] : false
            );
        }

        $ids['title'] = $this->DB_Products->get_product_ids_from_titles(
            !empty($atts['title']) ? $atts['title'] : false
        );
        $ids['vendor'] = $this->DB_Products->get_product_ids_from_vendors(
            !empty($atts['vendor']) ? $atts['vendor'] : false
        );
        $ids[
            'description'
        ] = $this->DB_Products->get_product_ids_from_description(
            !empty($atts['description']) ? $atts['description'] : false
        );
        $ids['product_type'] = $this->DB_Products->get_product_ids_from_types(
            !empty($atts['product_type']) ? $atts['product_type'] : false
        );
        $ids['slug'] = $this->DB_Products->get_product_ids_from_handles(
            !empty($atts['slug']) ? $atts['slug'] : false
        );
        $ids['tag'] = $this->DB_Tags->get_product_ids_from_tag(
            !empty($atts['tag']) ? $atts['tag'] : false
        );
        $ids['post_id'] = $this->DB_Products->get_product_ids_from_post_ids(
            !empty($atts['post_id']) ? $atts['post_id'] : false
        );
